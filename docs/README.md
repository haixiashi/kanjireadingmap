...
`build.py` and inlined into the JS. See `build/kanjimap_processed.js` for
current values.
...
lives in `src/data.json`; all other files are derived from it.
...
### rebuild_snapshot.py
Full rebuild pipeline for data.json. Runs four phases:
...
### build.py
Builds index.html from `src/kanjimap.js`, `src/styles.css`, and `src/data.json`:
...
### reencode_bac.py
Encodes data.json into the D string using binary arithmetic coding
...
compares every entry against src/data.json. Run after any data or
encoding change.
