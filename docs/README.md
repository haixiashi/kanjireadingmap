...
### kanjimap.js
The uncompressed JS+CSS source. Contains variable placeholders for
probability model values (e.g. `Z(CP)`, `Z(K1)`, `KP[pt-1]`) that
`build.py` replaces with computed literals at build time. Edit this
file for JS/UI changes, then run `build.py` to rebuild.

### kanjimap_processed.js
Generated copy of `kanjimap.js` with all placeholders replaced by
their current computed values. For reference only — not used by the
build. Regenerated by `build.py`.

### build.py
Builds index.html from `kanjimap.js`:
1. Computes probability models from `snapshot.json`
2. Replaces variable placeholders in JS with computed values
3. Deflate-raw compresses the JS payload
4. Encodes as base-93 (F string)
5. Reads D string from existing index.html
6. Assembles final HTML with bootstrap
